  ...
  const fetchUnits = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          err instanceof Error ? err.message : "Gagal memuat Production Unit",
        variant: "destructive",
      });
    } finally {
  ...
  const printBarcode = async (unitId: number, uniqCode: string) => {
    ...
    `;
      ...
      PROCESSES.forEach((process) => {
        const g = genUnits.find(
          (gu: { process: string }) => gu.process === process
        );
        if (g) {
          content += `
          <div class="block">
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Menampilkan semua unit. Barcode bisa di-print/download setelah
            digenerate.
          </CardDescription>
        </CardHeader>
